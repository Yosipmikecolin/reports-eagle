...
import { generateCarrier, generateH6 } from "@/reports";
...
export default function Page() {
  ...
  useEffect(() => {
    const reportString = localStorage.getItem("h6");
  ...
  const generatePDF = async (report: any) => {
    try {
      const url = await generateH6(report, "Dispositivos en estado vigencia");

      const fileName = `dispositivos_vigencia${report.carrier?.folio || "N/A"}.pdf`;

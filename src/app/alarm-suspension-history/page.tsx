...
import { generateH9 } from "@/reports";
...
export default function Page() {
  ...
  useEffect(() => {
    const reportString = localStorage.getItem("h9");
  ...
  const generatePDF = async (report: any) => {
    try {
      const url = await generateH9(
        report,
        "Historial de Suspensión de Alarmas"
      );

      const fileName = "historial_suspensión.pdf";

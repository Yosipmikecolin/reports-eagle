...
export default function FormProximasInstalaciones() {
  ...
  const handleGenerateReport = () => {
    saveReport("Próximas Instalaciones", formData);
    localStorage.setItem("h13", JSON.stringify(formData));
    ...
    <Card>
      <CardHeader>
        <div className="flex items-center justify-between">
          ...
          <Link href="/upcoming-installations" target="_blank">
            <Link2 />
          </Link>
        </div>
      </CardHeader>
      <CardContent className="space-y-6">
        ...
        <div className="space-y-2">
          <Label htmlFor="fechaHoraInstalacion">
            Fecha y hora instalación programada
          </Label>
          <Input
            id="fechaHoraInstalacion"
            type="datetime-local"
            value={formData.fechaHoraInstalacion}
            onChange={handleInputChange}
          />
        </div>
      </CardContent>
